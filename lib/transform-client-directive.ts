...
import type { BabelTypes, ImportElement } from './types'

export function transformClientDirective(
  t: BabelTypes,
  path: NodePath<babel.types.JSXNamespacedName>,
  imports: ImportElement[],
) {
  if (path.node.name.name === 'load') {
    return transformClientLoad(t, path, imports)
  }
}

function transformClientLoad(
  t: BabelTypes,
  path: NodePath<babel.types.JSXNamespacedName>,
  imports: ImportElement[],
) {
